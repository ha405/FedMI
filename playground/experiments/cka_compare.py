...
class CKACompareExperiment(BaseExperiment):
    def run(self):
        args = self.args
        exp_dir_a = args.exp_a
        exp_dir_b = args.exp_b if args.exp_b else args.exp_a
        
        cfg_a = load_config(exp_dir_a)
        cfg_b = load_config(exp_dir_b)
        ...
        model_a = load_model(exp_dir_a, cfg_a, round_num=args.round)
        model_b = load_model(exp_dir_b, cfg_b, round_num=args.round)
        
        if args.mode == "prehead":
            self.print_header(f"Pre-head Latent CKA: {exp_dir_a} vs {exp_dir_b}")
            ...
            circuits_a = load_circuits(exp_dir_a, source=args.source, round_key=args.round_key or "last")
            circuits_b = load_circuits(exp_dir_b, source=args.source, round_key=args.round_key or "last")
            
            client_key_a = f"client_{args.client_a}"
            client_key_b = f"client_{args.client_b}"
            
            if client_key_a not in circuits_a:
                sys.exit(f"[ERROR] {client_key_a} not found in {exp_dir_a}.")
                
            if client_key_b not in circuits_b:
                sys.exit(f"[ERROR] {client_key_b} not found in {exp_dir_b}.")
            ...
            self.print_header(f"CKA Circuit Comparison: {exp_dir_a} ({client_key_a}) vs {exp_dir_b} ({client_key_b})")
...
def add_args(subparsers):
    ...
    p.add_argument("--exp_a", required=True, help="First experiment directory")
